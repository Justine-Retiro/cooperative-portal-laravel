...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST["role"])) {
        $role = $_POST["role"];
        $queryRole = "UPDATE users SET role = ? WHERE account_number = ?";
        $stmtRole = $conn->prepare($queryRole);
        $stmtRole->bind_param("ss", $role, $account_number);
        $stmtRole->execute();
    }
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $queryRole = "SELECT role FROM users WHERE account_number = ?";
        $stmtRole = $conn->prepare($queryRole);
        $stmtRole->bind_param("s", $account_number);
        $stmtRole->execute();
        $resultRole = $stmtRole->get_result();
        $rowRole = $resultRole->fetch_assoc();
    
        // Redirect to editAdmin.php
        header('Location: ../edit/editAdmin.php?account_number=' . $account_number . '&role=' . $rowRole["role"]);
